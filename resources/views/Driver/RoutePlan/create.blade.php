                ...
                <div class="col-sm-12">
                    <div class="card-box">
                        <ol class="breadcrumb">
                            ...
                            <li>
                                <a href="{{route('route_plan.index')}}">Route Plan Management</a>
                            </li>
                            <li class="active">
                                Create
                            </li>
                        ...
                        {{ Html::ul($errors->all()) }}
                        {{ Form::open(array('url' => route('route_plan.store'), 'role' => 'form')) }}
                        ...
                        <div class="col-md-4">
                            <div class="form-group">
                                {{ Form::label('delivery_date', 'Delivery Date', array('class' => 'col-2 col-form-label')) }}
                                <small class="form-text text-muted">
                                    <span class="text-danger">*</span> Isi sesuai jumlah outlet, tidak boleh
                                    dikosongkan.
                                </small>
                        ...
                        <div class="form-group">
                            <div class="col-10">
                                ...
                                <a class="btn btn-danger m-t-20" href="{{ route('route_plan.index') }}"><i
    ...
    <script>
        $('document').ready(function () {
                ...
                ajax: {
                    ...
                    processResults: function (data, params) {
                        ...
                        const items = $.map(data.data, function (item) {
                            return {id: item.id.toString(), text: `${item.qrcode} ${item.brand}`};
                        });
                        return {
                            results: items,
                            pagination: {
            ...
            $('#date-range').datepicker({
                ...
                url = "{{route("geo.outlet")}}" + "?city_id=" + $('select#cities').val() + "&start=" + $('#date-range [name="start"]').val() + "&end=" + $('#date-range [name="end"]').val();
                $.ajax({
                    url: url,
                    method: "GET",
                    success: function (data) {
