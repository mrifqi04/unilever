            ...
            <div class="row">
                <div class="col-sm-12">
                    <div class="card-box">
                        ...
                        <div class="form-group">
                            {{ Form::label('listAdmin', 'Data Juragan', array('class' => 'col-2 col-form-label')) }}
                            <div class="col-10">
                                <table class="table table-hover" id="data-table">
                                    ...
                                    <tbody id="listadmin">
                                        @foreach($current_juragan as $i)
                                            @php
                                                $juraganData = App\Models\JuraganManagement\Juragan::find($i);
                                            @endphp
                                        <tr>
                                            <td><input type="checkbox" id="val{{$juraganData->id}}" name="id_submitted_outlets[]" value="{{$juraganData->id}}" checked onclick="return false;"/>&nbsp;</td>
                                            <td>{{$juraganData->id}}</td>
                                            <td>{{$juraganData->unilever_id}}</td>
                                            <td>{{$juraganData->name}}</td>
                                            <td>{{$juraganData->email}}</td>
                                            <td>{{$juraganData->phone}}</td>
                                        </tr>
                                        @endforeach
                                    </tbody>
                                </table>
