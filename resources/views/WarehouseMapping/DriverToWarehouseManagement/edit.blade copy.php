            ...
            <div class="row">
                <div class="col-sm-12">
                    <div class="card-box">
                        ...
                        <div class="form-group">
                            <div class="col-10">
                                <button type="button" class="btn waves-effect waves-light btn-rounded btn-sm btn-success m-t-20 pull-right" data-toggle="modal" data-target="#create">Update</button>
...
@push('scripts')
    ...
    <script>
            ...
            $(document).ready(function () {
                $.ajax({
                    ...
                    success: function (data) {
                        ...
                        loop_driver_selected = id_driver_mappings.split(",");
                        if(loop_driver_selected.length > 0){
                            $.each( loop_driver_selected, function( index, value ) {
                        ...
                        data_table.DataTable().on( 'draw', function () {
                            if($('#check-all:checked').val()){
                                $('#listdriver input:checkbox').not(this).prop('checked', true);
                            }
                        });
                    }
                });
        ...
        $(document).on('change', '#listdriver input:checkbox', function() {
            if(!this.checked){
                if($('#check-all:checked').val()){
                    $('#check-all').prop('checked', false);
                    $('#listdriver input:checkbox').not(this).prop('checked', false);
