                ...
                <div class="col-sm-12">
                    <div class="card-box">
                        <ol class="breadcrumb">
                            ...
                            <li class="active">
                                <a href="{{route('unilever.index')}}">Approval Deploy Kabinet</a>
                            </li>
                        </ol>
                        ...
                        @if($canExportOutlet)
                            <a href="{{route('unilever.export-outlet')}}" class="btn btn-primary  d-none d-lg-block">
                        ...
                        <div class="m-t-20">
                            <div class="collapse {{$in_search === true ? 'in' : ''}}"
                                ...
                                <div class="card-box">
                                    <div class="card-view">
                                        {{ Form::open(['url' => route('unilever.index'), 'role' => 'form', 'method' => 'get','class'=>'form-inline', 'id' => 'form-filter']) }}
                                        <div class="row">
                                        ...
                                        <div class="row m-t-10">
                                            <div class="form-group col-md-3">
                                                <div class="col-10">
                                                    <input class="form-control date" name="send_date"
                                                        id="send_date"
                                                        value="{{app('request')->input('send_date')}}"
                                                        placeholder="Tgl Jadwal Kirim">
                                                </div>
                                            </div>
                                        </div>
                                        <div class="row m-t-10">
        ...
        $(document).ready(function() {
            ...
            function loadDataTable() {
                data_table = $('#data-table').DataTable({
                    ...
                    "serverSide" : true,
                    "ajax" : {
                        "url" : "{{route("unilever.index")}}?get_records=1&"+$('#form-filter').serialize(),
                        "type": 'GET',
                    },
                    "createdRow": function ( row, data, index ) {
                        var td = [];
                        td[0] = null;
                        if (data['section'] == 'juragan' && data['status_progress'] == '3') {
                        ...
                        td[8] = data['address'];
                        td[9] = data['phone'];
                        td[10] = `<span class="label label-`+data['status_class']+`"style="padding: 5px">`+data['value']+`</span>`;
                        td[11] = data['created_date_format'];
                        td[12] = data['recommend_date_format'];
                        td[13] = data['send_date_format'];
                        td[14] = '';
                        if ("{{$canShow}}") {
                            td[14] += `<a href="{{url('unilever')}}/`+data['outlet_id']+`"
                                title="show"
                        ...
                        if ("{{$canApprove}}") {
                            td[14] += `<form name="form-approve-`+data['id']+`"
                                    action="{{url('unilever/approve')}}/`+data['id']+`"
                                    method="POST">
                                {{ csrf_field() }}
                                {{ method_field('PUT') }}
                        ...
                        if ("{{$canReject}}") {
                            td[14] += `<form name="form-reject-`+data['id']+`"
                                    action="{{url('unilever/reject')}}/`+data['id']+`"
                                    method="POST">
                                {{ csrf_field() }}
                                {{ method_field('PUT') }}
                                ...
                                <div class="modal-body">
                                    <div class="row">
                                        <form name="form-delivery-date-`+data['id']+`"
                                            action="{{url('unilever/change-delivery-date')}}/`+data['id']+`"
            ...
            if (selectedIds.length > 0) {
                ...
                function (isConfirm) {
                    if (isConfirm) {
                        $.ajax({
                            method: 'put',
                            url: "{{route('unilever.approve-bulk')}}",
                            dataType: 'json',
                            data: {
                                ids: selectedIds,
                                _token: '{{csrf_token()}}',
            ...
            if (selectedIds.length > 0) {
                ...
                function (isConfirm) {
                    if (isConfirm) {
                        $.ajax({
                            method: 'put',
                            url: "{{route('unilever.reject-bulk')}}",
                            dataType: 'json',
                            data: {
                                ids: selectedIds,
                                _token: '{{csrf_token()}}',
