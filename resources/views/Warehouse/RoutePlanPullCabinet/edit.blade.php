            ...
            <div class="row">
                <div class="col-sm-12">
                    <div class="card-box">
                        ...
                        {{ Html::ul($errors->all()) }}
                        {{ Form::open(array('url' => route('warehouse.route-plan-pull-cabinet.update', ['id' => $data->id]), 'role' => 'form', 'method' => 'PUT')) }}

                        <div class="form-group">
                            {{ Form::label('provinces', 'Province', array('class' => 'col-2 col-form-label')) }}
                            <div class="col-10">
                                {{ Form::select('provinces', $provinces, $data->deliveryOrders->id_province, ['placeholder' => 'Pilih Province', 'class' => 'form-control', 'required' => 'required']) }}
                            ...
                            <div class="col-10 m-t-10">
                                <div class="input-group">
                                    <input type="text" class="form-control" placeholder="yyyy-mm-dd"
                                            name="date-tarik" id="date-tarik" required value="{{\Carbon\Carbon::parse($data->outlet->mapOutlet->outletRetractionProgress->first()->send_date)->format('Y-m-d')}}">
                            ...
                            <div class="col-md-4">
                                <div class="form-group">
                                    ...
                                    <div class="col-10 m-t-10">
                                        <div class="input-group">
                                            <input type="text" class="form-control" placeholder="yyyy-mm-dd"
                                                name="reportrange[]" required value="{{\Carbon\Carbon::parse($data->start_date)->format('Y-m-d')}}">
                                            <span class="input-group-addon bg-custom b-0 text-white"><i
                                                        class="icon-calender"></i></span>
                                        </div><!-- input-group -->
                                    </div>
    ...
    <script>
        $('document').ready(function () {
            ...
            $('select#cities').on("change", function () {
                getListOutlet();
                if ($('select#cities').val() == "{{$data->deliveryOrders->id_city}}") {
                    $('#date-tarik').change();
                }
            });
        ...
        function getListOutlet() {
            ...
            url = '{{route("geo.outletRetraction")}}' 
                + "?city_id=" + $('select#cities').val()
                + "&date_tarik=" + $('#date-tarik').val()
                + "&id_route_plan={{$data->id}}";
            $.ajax({
                url: url,
                method: "GET",
                success: function (data) {
                    ...
                    if ($('select#cities').val() == "{{$data->deliveryOrders->id_city}}"
                        && $('#date-tarik').val() == "{{\Carbon\Carbon::parse($data->outlet->mapOutlet->outletRetractionProgress->first()->send_date)->format('Y-m-d')}}") {
                            $('select.outlet').val("{{ $data->id_outlet }}").change();
                    }

                    if ($('select.outlet').val() == '{{ $data->id_outlet }}') {
                        $('select.cabinet').append('<option value="{{$data->id_cabinet}}" selected>{{ $data->cabinet->qrcode_by_auditor }} {{ $data->cabinet->brand }}</option>');
                    } else {
                        $("select.cabinet").empty().select2({placeholder: "Select Cabinet"});
                    }
                }
            });
        }

        function fillSelectData(selectorId, datas, placeholder, isNotTrigger) {
        ...
        $("select#provinces").on("change", function (event) {
            ...
            $.ajax({
                ...
                success: function (data) {
                    ...
                    if (val == "{{$data->deliveryOrders->id_province}}") {
                        fillSelectData("select#cities", items, 'Select Cities', 1);
                        $('select#cities').val("{{ $data->deliveryOrders->id_city }}").change();
                    } else {
