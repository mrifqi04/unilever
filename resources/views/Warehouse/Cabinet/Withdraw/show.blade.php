                    ...
                    <div class="profile-detail card-box">
                        <div>
                            ...
                            @if($survey != null)
                                ...
                                @php($sr = json_decode($survey->answers))
                                @php($exist =($sr->kulkas->exist == '1')?'Ya':'Tidak')
                                @php($perdana =($sr->perdana == '1')?'Ya':'Tidak')
                                @php($freezer =($sr->freezer == '1')?'Ya':'Tidak')
                                @php($selling =($sr->selling->selling == '1')?'Ya':'Tidak')
                                @php($sell_name =(!empty($sr->selling->name))?', '.implode(",", $sr->selling->name):'')
                                <div class="text-left">
                                    <p class="text-muted font-13"><strong>Jenis Toko :</strong> <span
                                                class="m-l-15">{{$sr->id_outlet_type}}</span>
                                    </p>

                                    <p class="text-muted font-13"><strong>Status Kepemilikan :</strong><span
                                                class="m-l-15">{{$sr->id_ownership_status}}</span>
                                    </p>

                                    <p class="text-muted font-13"><strong>Tipe Jalan :</strong> <span
                                                class="m-l-15">{{$sr->id_street_type}}</span>
                                    </p>

                                    <p class="text-muted font-13"><strong>Radius 50 - 100 dekat dengan :</strong> <span
                                                class="m-l-15">{{$sr->area_radius}}</span>
                                    </p>

                                    <p class="text-muted font-13"><strong>Sudah berjualan es krim? :</strong><span
                                                class="m-l-15">{{$selling}} {{$sell_name}}</span>
                                    </p>
                                    <p class="text-muted font-13"><strong>Ada kulkas di rumah? :</strong> <span
                                                class="m-l-15">{{$exist}},{{$sr->kulkas->type}} Pintu</span>
                                    </p>

                                    <p class="text-muted font-13"><strong>Bersedia membeli Paket Perdana? :</strong>
                                        <span class="m-l-15">{{$perdana}}</span>
                                    </p>

                                    <p class="text-muted font-13"><strong>Tersedia tempat untuk Cabinet :</strong> <span
                                                class="m-l-15">{{$freezer}}</span>
                                    </p>

                                    <p class="text-muted font-13"><strong>Kapasitas Listrik (VA) :</strong> <span
                                                class="m-l-15">{{$sr->electricity_capacity}}</span>
                                    </p>

                                    <p class="text-muted font-13"><strong>Frekuensi Mati Listrik dalam 1 bulan
                                            :</strong> <span class="m-l-15">{{$sr->blackout_intensity}}</span>
                                    </p>

                                    <p class="text-muted font-13"><strong>Jadwal pengiriman Cabinet :</strong> <span
                                                class="m-l-15">{{date('l, j F Y', strtotime($data->start_date))}}</span>
                                    </p>
                                </div>
                            @endif
                        </div>
                        ...
                        <div class="p-20">
                            ...
                            <div class="nicescroll p-l-r-10" style="max-height: 555px;">
                                <div class="timeline-2">
                                    {{--                                    @if(count($data->outlet->mapOutlet->outletProgress) > 0)--}}
                                    @foreach($data->outlet->mapOutlet->outletProgress as $op)
                                        <div class="time-item">
                                            <div class="item-info">
                                                <div class="text-muted">
                                                    <small>{{\Carbon\Carbon::parse($op->created_at)->format('M d Y H:i:s')}}</small>
                                                </div>
                                                <p><span class="text-capitalize font-bold">{{$op->section}}</span></p>
                                                <p>
                                                    Status
                                                    @if($op->section == 'hunter')
                                                        @switch($op->status_progress)
                                                            @case("1")
                                                            <em class="font-bold text-success">Deal</em>
                                                            @break
                                                    ...
                                                    @if($op->section == 'driver')
                                                        @switch($op->status_progress)
                                                            ...
                                                            @case("7")
                                                            <em class="font-bold text-success">Sedang Dikirim</em>
                                                            @break
                                                        @endswitch
                                                    @endif
                                                </p>
...
@push('scripts')
    ...
    <script>
        ...
        function initMap() {
            var myLatLng = {lat: {{$data->outlet->latitude}}, lng: {{$data->outlet->longitude}}};
