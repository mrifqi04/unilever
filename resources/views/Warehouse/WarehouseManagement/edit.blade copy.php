...
@push('scripts')
    ...
    <script>
        ...
        var id_warehouse_admin = "{{$warehouseManagement->id_warehouse_admins}}";
    </script>
    {{-- <script>
    ...
    <script>
        $(function () {
            $(document).ready(function () {
                $.ajax({
                    type: 'GET',
                    url: "{{url('/warehouse-management/get-admin-edit')}}",
                    dataType: 'json',
                    success: function (data) {
                        $('#check-all').prop('checked', false);
                        data_table.DataTable().destroy();
                        $('#listadmin').html(data);
                        loop_admin_selected = id_warehouse_admin.split(",");
                        if(loop_admin_selected.length > 0){
                            $.each( loop_admin_selected, function( index, value ) {
                                $('[name="id_warehouse_admins[]"][value="'+value+'"]').prop('checked', true);
                            });
                        }
                        data_table.DataTable({
                            "paging":   false,
